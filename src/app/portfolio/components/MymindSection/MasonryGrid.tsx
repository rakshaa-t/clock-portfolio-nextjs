...
type MasonryGridProps = {
  ...
  className?: string
  /** Number of cards that were initially visible (extras get no whileInView) */
  initialCount?: number
}
 ...
 */
function MasonryGrid({ cards, activeIdx, onCardClick, className, initialCount }: MasonryGridProps) {
  const isMobile = useMatchMedia('(max-width: 480px)')

  const columns = useMemo(() => {
    const colCount = isMobile ? 2 : 3
    const cols: { card: MymindCardType; globalIdx: number; flatIdx: number }[][] = Array.from({ length: colCount }, () => [])
    ...
    cards.forEach((card, flatIdx) => {
      ...
      cols[shortest].push({ card, globalIdx, flatIdx })
    ...
    >
      {columns.map((col, colIdx) => (
        <div key={colIdx} className="flex flex-col gap-2">
          {col.map(({ card, globalIdx, flatIdx }, cardIdx) => {
            const isExtra = initialCount != null && flatIdx >= initialCount
            return (
              <motion.div
                key={`${colIdx}-${cardIdx}`}
                {...(!isExtra ? {
